    ...
    def _select_indices(self, model: LLL_Net, sel_loader: DataLoader, exemplars_per_class: int, transform) -> Iterable:
        ...
        with torch.no_grad():
            ...
            for images, targets in sel_loader:
                images_all = (images.to(model_device), images.to(model_device))
                feats_all = model(images_all, return_features=True)[1]
                # feats, _ = torch.split(feats_all, split_size_or_sections=[images.shape[0], images.shape[0]], dim=0)
                feats = feats_all[0:images.shape[0]]
...
class EntropyExemplarsSelector(ExemplarsSelector):
    ...
    def _select_indices(self, model: LLL_Net, sel_loader: DataLoader, exemplars_per_class: int, transform) -> Iterable:
        ...
        with torch.no_grad():
            model.eval()
            for images, targets in sel_loader:
                extracted_logits.append(torch.cat(model((images.to(model_device), images.to(model_device))), dim=1))
