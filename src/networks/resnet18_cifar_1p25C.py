...
class BasicBlock(nn.Module):
    ...
    def __init__(self, inplanes, planes, stride=1, downsample=None):
        ...
        seq_ratio = 1.25
        planes_2x = int(seq_ratio * planes)
        self.conv1_layers = [
            # The 1-st passage for Conv1.
            ("conv1_new_1_1", nn.Conv2d(inplanes, planes_2x, 1, stride=1, padding=0, bias=False)),
            ("bn1_new_1_1", nn.BatchNorm2d(planes_2x)),
            ("conv1_new_1_2", nn.Conv2d(planes_2x, planes_2x, 3, stride=stride, padding=1, bias=False,
                                groups=planes_2x)),
            ("bn1_new_1_2", nn.BatchNorm2d(planes_2x)),
            ("conv1_new_1_3", nn.Conv2d(planes_2x, planes, 1, stride=1, padding=0, bias=False)),
            ("bn1_new_1_3", nn.BatchNorm2d(planes)),
            # The 2-nd passage for Conv1.
            ("conv1_new_2_1", nn.Conv2d(inplanes, planes_2x, 1, stride=1, padding=0, bias=False)),
            ("bn1_new_2_1", nn.BatchNorm2d(planes_2x)),
            ("conv1_new_2_2", nn.Conv2d(planes_2x, planes_2x, 3, stride=stride, padding=1, bias=False,
                                groups=planes_2x)),
            ("bn1_new_2_2", nn.BatchNorm2d(planes_2x)),
            ("conv1_new_2_3", nn.Conv2d(planes_2x, planes, 1, stride=1, padding=0, bias=False)),
        ...
        self.conv2_layers = [
            ...
            ("conv2_new_1_1", nn.Conv2d(planes, planes_2x, 1, stride=1, padding=0, bias=False)),
            ("bn2_new_1_1", nn.BatchNorm2d(planes_2x)),
            ("conv2_new_1_2", nn.Conv2d(planes_2x, planes_2x, 3, stride=1, padding=1, bias=False,
                                groups=planes_2x)),
            ("bn2_new_1_2", nn.BatchNorm2d(planes_2x)),
            ("conv2_new_1_3", nn.Conv2d(planes_2x, planes, 1, stride=1, padding=0, bias=False)),
            ("bn2_new_1_3", nn.BatchNorm2d(planes)),
            # The 2-nd passage for Conv2.
            ("conv2_new_2_1", nn.Conv2d(planes, planes_2x, 1, stride=1, padding=0, bias=False)),
            ("bn2_new_2_1", nn.BatchNorm2d(planes_2x)),
            ("conv2_new_2_2", nn.Conv2d(planes_2x, planes_2x, 3, stride=1, padding=1, bias=False,
                                groups=planes_2x)),
            ("bn2_new_2_2", nn.BatchNorm2d(planes_2x)),
            ("conv2_new_2_3", nn.Conv2d(planes_2x, planes, 1, stride=1, padding=0, bias=False)),
...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000):
        ...
        seq_ratio = 1.25
        planes_2x = int(seq_ratio * self.output_channel)
        self.conv_stem_layers = [
            # The 1-st passage for Conv1.
            ("conv1_new_1_1", nn.Conv2d(self.input_channel, planes_2x, 1, stride=1, padding=0, bias=False)),
            ("bn1_new_1_1", nn.BatchNorm2d(planes_2x)),
            ("conv1_new_1_2", nn.Conv2d(planes_2x, planes_2x, 3, stride=1, padding=1, bias=False,
                                groups=planes_2x)),
            ("bn1_new_1_2", nn.BatchNorm2d(planes_2x)),
            ("conv1_new_1_3", nn.Conv2d(planes_2x, self.output_channel, 1, stride=1, padding=0, bias=False)),
            ("bn1_new_1_3", nn.BatchNorm2d(self.output_channel)),
            # The 2-nd passage for Conv1.
            ("conv1_new_2_1", nn.Conv2d(self.input_channel, planes_2x, 1, stride=1, padding=0, bias=False)),
            ("bn1_new_2_1", nn.BatchNorm2d(planes_2x)),
            ("conv1_new_2_2", nn.Conv2d(planes_2x, planes_2x, 3, stride=1, padding=1, bias=False,
                                groups=planes_2x)),
            ("bn1_new_2_2", nn.BatchNorm2d(planes_2x)),
            ("conv1_new_2_3", nn.Conv2d(planes_2x, self.output_channel, 1, stride=1, padding=0, bias=False)),
...
def resnet18_cifar_1p25C(pretrained=False, **kwargs):
