...
from .Distill_loss import Distill_Loss
...
class Appr(Inc_Learning_Appr):
    ...
    def __init__(self, model, device, network, nepochs=160, lr=0.1, decay_mile_stone=[80,120], lr_decay=0.1, clipgrad=10000,
                 ...
                 first_task_lr=0.1, first_task_bz=128, bal_ft=False, buffer_size=2000, minibatch_size_1=16, minibatch_size_2=128):
    ...
    def pre_train_process(self, t, trn_loader):
        ...
        # ------------------------------------------------------------------------------------------------------
        # Add by NieX in 10_17.
        if self.network == 'resnet18_cifar' or self.network == 'resnet18_cifar_conv1' or self.network == 'resnet18_cifar_conv1_s' or self.network == 'resnet18_cifar_group' or self.network == 'resnet18_cifar_group_s' or self.network == 'resnet18':
    ...
    def train_loop(self, t, trn_loader, val_loader):
        ...
        if len(self.exemplars_dataset) > 0 and t > 0:
            ...
            # --------------------------------------------------------------------------------------------------
            # Add for removing exemlars by NieX.
            # dset = trn_loader.dataset
            # --------------------------------------------------------------------------------------------------
    ...
    def train_epoch(self, t, trn_loader):
        ...
        for images, targets in trn_loader:
            ...
            # Distill_Loss
            outputs, _, pod_features = self.model((images, images), return_features=True)
            loss = Distill_Loss()(pod_features)
    ...
    #             images_all = torch.cat((images, buf_inputs), dim=0)
    #             outputs, features, _ = self.model((images_all, buf_inputs), return_features=True)
    ...
    #             targets = torch.cat((targets, buf_outputs), dim=0)
        ...
        if ref_outputs is None or ref_features is None:
            ...
            if self.margin_ranking:
                ...
                if hard_num > 0:
                    ...
                    loss_mr = nn.MarginRankingLoss(margin=self.dist)(gt_scores.view(-1, 1),max_novel_scores.view(-1, 1),torch.ones(hard_num * self.K).to(self.device).view(-1, 1))
