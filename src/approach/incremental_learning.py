    ...
    def eval(self, t, val_loader):
        ...
        with torch.no_grad():
            ...
            for images, targets in val_loader:
                ...
                # Forward current model
                # ----------------------------------------------------------------------------
                images = (images.to(self.device), images.to(self.device))
                outputs = self.model(images)
                
                for i in range(len(outputs)):
                    outputs_1, outputs_2 = torch.split(outputs[i], split_size_or_sections=[targets.shape[0], targets.shape[0]], dim=0)
                    # outputs[i] = torch.maximum(outputs_1, outputs_2)
                    # outputs[i] = (outputs_1 + outputs_2) / 2
                    outputs[i] = outputs_1
