    ...
    def train_epoch(self, t, trn_loader):
        ...
        for images, targets in trn_loader:
            ...
            if self.buffer.is_empty() or t == 0:
                ...
                buf_outputs = []
                buf_inputs_br1, buf_labels_br1 = self.buffer.get_data(self.minibatch_size_1, transform=None)
                images = torch.cat((images, buf_inputs_br1), dim=0)
                targets = torch.cat((targets, buf_labels_br1), dim=0)
                buf_inputs_br2, buf_labels_br2 = self.buffer.get_data(self.minibatch_size_2, transform=None)

                # outputs, features, _ = self.model((images, buf_inputs), return_features=True)
                # import pdb
                # pdb.set_trace()

                outputs, features, _ = self.model((images, buf_inputs_br2), return_features=True)
                ...
                # loss += nn.CrossEntropyLoss()(buf_outputs, buf_labels_br2)
                loss = loss + self.l_beta*nn.CrossEntropyLoss()(buf_outputs, buf_labels_br2)
